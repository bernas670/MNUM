...
double diff_f1(double x, double y) { return -y + 4 * x - 5; }

double f2(double x, double y) { return x + 3 * log(x) - pow(y, 2); }
double diff_f2(double x, double y) { return -2 * y; }
...
void newton(double x_guess, double y_guess) {
  ...
  do
  {
    ...
    xn = x_guess - f1(x_guess, y_guess) / diff_f1(x_guess, y_guess);
    yn = y_guess - f2(x_guess, y_guess) / diff_f2(x_guess, y_guess);
